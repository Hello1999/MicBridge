...
> 2026-09-22 范围更新：快捷指令协议保持兼容，控制对象为 Android 系统麦克风。本文旧场景中的 ChatGPT Live 仅是声学测试应用示例，可换为其他能持续收音的应用；不是安装、包名或权限前提。新版验收见 [系统麦克风审查](SYSTEM_MIC_REVIEW_ZH.md)。
...
Android 还会在成功的真实状态切换后播放独立电子提示音：开放为上扬双音，屏蔽为下行双音，媒体音量在播放期间临时使用最接近 30% 的系统档位并在结束后恢复。它不替代快捷指令的 1/2/3 次触感判断；失败、重放或状态没有变化时不会响。
...
- 查看 Android 的 MicBridge 常驻通知或状态页。Action Button 的 OPEN 不再自动到期；若结果未知，不要盲目重按，因为新 request ID 会再次执行 toggle。应先恢复网络并从 Android 状态源确认当前状态。
...
| 成功执行后响应丢失 | 在响应阶段断网 | 状态标记未知；不立即重按；从 Android 状态页确认，或用明确的 `/block` 安全收尾 | 待真机验证 |
...
- Action Button 的 toggle 是锁存状态：第一次成功开放后不会按秒数自动屏蔽，第二次成功触发才切回 `blocked`。5–30 秒时限仅用于 Android 声学校准和诊断 `/open`；服务重启、监督器/权限异常或受监控网络变化仍会故障安全屏蔽。
